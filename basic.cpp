...
PROGMEM const TokenTableEntry tokenTable[] = {
    ...
    {"LIST",TKN_FMT_POST}, {"RUN",TKN_FMT_POST}, {"GOTO",TKN_FMT_POST}, {"REM",T
KN_FMT_POST},
    {"STOP",TKN_FMT_POST}, {"INPUT",TKN_FMT_POST},  {"CONT",TKN_FMT_POST}, {"IF"
,TKN_FMT_POST},
    {"THEN",TKN_FMT_PRE|TKN_FMT_POST}, {"LEN",1|TKN_ARG1_TYPE_STR}, {"VAL",1|TKN
_ARG1_TYPE_STR}, {"RND",0},
    {"INT",1}, {"STR$", 1|TKN_RET_TYPE_STR}, {"FOR",TKN_FMT_POST}, {"TO",TKN_FMT
_PRE|TKN_FMT_POST},
    {"STEP",TKN_FMT_PRE|TKN_FMT_POST}, {"NEXT", TKN_FMT_POST}, {"MOD",TKN_FMT_PR
E|TKN_FMT_POST}, {"NEW",TKN_FMT_POST},
    {"GOSUB",TKN_FMT_POST}, {"RETURN",TKN_FMT_POST}, {"DIM", TKN_FMT_POST}, {"LE
FT$",2|TKN_ARG1_TYPE_STR|TKN_RET_TYPE_STR},
    {"RIGHT$",2|TKN_ARG1_TYPE_STR|TKN_RET_TYPE_STR}, {"MID$",3|TKN_ARG1_TYPE_STR
|TKN_RET_TYPE_STR}, {"CLS",TKN_FMT_POST}, {"PAUSE",TKN_FMT_POST},
    {"POSITION", TKN_FMT_POST},  {"PIN",TKN_FMT_POST}, {"PINMODE", TKN_FMT_POST}
, {"INKEY$", 0},
...
void stackAdd2Strs() {
    // equivalent to popping 2 strings, concatenating them and pushing the resul
t
    unsigned char *p = &mem[sysSTACKEND];
    ...
    p = &mem[sysSTACKEND];
    // shift the second string up (overwriting the null terminator of the first
string)
...
void stackLeftOrRightStr(int len, int mode) {
    // equivalent to popping the current string, doing the operation then pushin
g it again
    unsigned char *p = &mem[sysSTACKEND];
...
// +--------+-------+-----------------+----------+-------+ . . .+-------+-------
------+. .
// | len    | type  | name            | num dims | dim1  |      | dimN  | elem(1
,..1) |
...
int storeForNextVariable(char *name, float start, float step, float end, uint16_
t lineNum, uint16_t stmtNum) {
    int nameLen = strlen(name);
    int bytesNeeded = 3;        // len + flags
    bytesNeeded += nameLen + 1; // name
    ...
    // unlike simple numeric variables, these are reallocated if they already ex
ist
    // since the existing value might be a simple variable or a for/next variabl
e
    unsigned char *p = findVariable(name, VAR_TYPE_NUM|VAR_TYPE_FORNEXT);
    if (p != NULL) {
...
int createArray(char *name, int isString) {
    ...
    // keep the current stack position, since we'll need to pop these values aga
in
    ...
    // strings and arrays are re-allocated if they already exist
    unsigned char *p = findVariable(name, (isString ? VAR_TYPE_STR_ARRAY : VAR_T
YPE_NUM_ARRAY));
    if (p != NULL) {
        // check there will actually be room for the new value
        uint16_t oldVarLen = *(uint16_t*)p;
...
int setStrArrayElem(char *name) {
    ...
    // keep the current stack position, since we can't overwrite the value strin
g
    int oldSTACKEND = sysSTACKEND;
    ...
    *(uint16_t*)p1 += bytesNeeded;
    memmove(&mem[sysVARSTART - bytesNeeded], &mem[sysVARSTART], p - &mem[sysVARS
TART]);
...
int gosubStackPush(int lineNumber,int stmtNumber) {
    ...
    memmove(&mem[sysVARSTART]-bytesNeeded, &mem[sysVARSTART], sysVAREND-sysVARST
ART);
    sysVARSTART -= bytesNeeded;
...
int gosubStackPop(int *lineNumber, int *stmtNumber) {
    ...
    *stmtNumber = (int)*p;
    ...
    // shift the variable table
    memmove(&mem[sysVARSTART]+bytesFreed, &mem[sysVARSTART], sysVAREND-sysVARSTA
RT);
...
int nextToken()
{
    ...
    if (isalpha(*tokenIn)) {
        ...
        for (int i = FIRST_IDENT_TOKEN; i <= LAST_IDENT_TOKEN; i++) {
            if (strcasecmp(identStr, (char *)pgm_read_word(&tokenTable[i].token)
) == 0) {
        ...
        if  (dollarPos && dollarPos!= &identStr[0] + identLen - 1) return ERROR_
LEXER_UNEXPECTED_INPUT;
    ...
    for (int i=LAST_NON_ALPHA_TOKEN; i>=FIRST_NON_ALPHA_TOKEN; i--) {
        ...
        if (strncmp((char *)pgm_read_word(&tokenTable[i].token), (char*)tokenIn,
 len) == 0) {
            if (tokenOutLeft <= 1) return ERROR_LEXER_TOO_LONG;
            *tokenOut++ = i;
            tokenOutLeft--;
...
// stmt number is 0 for the first statement, then increases after each command s
eperator (:)
// Note that IF a=1 THEN PRINT "x": print "y" is considered to be only 2 stateme
nts
        ...
        case TOKEN_VAL:
            {
                ...
                int val = tokenize((unsigned char*)stackGetStr(), &mem[sysSTACKE
ND], sysVARSTART - sysSTACKEND);
                if (val) {
                    if (val == ERROR_LEXER_TOO_LONG) return ERROR_OUT_OF_MEMORY;
                    else return ERROR_IN_VAL_INPUT;
                }
                // set tokenBuffer to point to the new set of tokens on the stac
k
...
int getTokPrecedence() {
    ...
    if (curToken == TOKEN_LT || curToken == TOKEN_GT || curToken == TOKEN_LT_EQ
|| curToken == TOKEN_GT_EQ) return 20;
    if (curToken == TOKEN_MINUS || curToken == TOKEN_PLUS) return 30;
    else if (curToken == TOKEN_MULT || curToken == TOKEN_DIV || curToken == TOKE
N_MOD) return 40;
    else return -1;
}
...
int parseBinOpRHS(int ExprPrec, int lhsVal) {
    ...
    while (1) {
        ...
        // If this is a binop that binds at least as tightly as the current bino
p,
        ...
        {       // String operations
            if (BinOp == TOKEN_PLUS) {
                ...
                if (executeMode) {
                    ...
                    else if (BinOp == TOKEN_NOT_EQ && ret != 0) stackPushNum(1.0
f);
    ...
    {   // numeric variable
        ...
        if (executeMode) {
            if (isArray) {
                ...
                if (!storeNumVariable(ident, stackPopNum())) return ERROR_OUT_OF
_MEMORY;
            }
        }
    }
    ...
    {   // string variable
        ...
        if (executeMode) {
            if (isArray) {
                ...
                if (!storeStrVariable(ident, stackGetStr())) return ERROR_OUT_OF
_MEMORY;
                stackPopStr();
            }
        }
...
int parse_FOR() {
    ...
    if (executeMode) {
        if (!storeForNextVariable(ident, start, step, end, lineNumber, stmtNumbe
r)) return ERROR_OUT_OF_MEMORY;
    }
    return 0;
}
...
int parse_NEXT() {
    ...
    if (executeMode) {
        ...
        // loop?
        if ((data.step >= 0 && data.val <= data.end) || (data.step < 0 && data.v
al >= data.end)) {
...
int parseStmts()
{
    ...
    while (ret == 0) {
        if (curToken == TOKEN_EOL)
            break;
        if (executeMode)
            sysSTACKEND = sysSTACKSTART = sysPROGEND;   // clear calculator stac
k
        int needCmdSep = 1;
        switch (curToken) {
        ...
        if (ret || breakCurrentLine || jumpLineNumber || jumpStmtNumber)
            break;
        // it should either be the end of the line now, and (generally) a comman
d seperator
...
int processInput(unsigned char *tokenBuf) {
    // first token can be TOKEN_INTEGER for line number - stored as a float in n
umVal
    ...
    if (gotLineNumber) {
        ...
        while (1) {
            ...
            if (!lineNumber && !jumpLineNumber && jumpStmtNumber) {
                // we're executing the buffer, and need to jump stmt (e.g. for/n
ext)
                tokenBuffer = tokenBuf;
            }
            else {
                ...
                if (jumpLineNumber || jumpStmtNumber) {
                    ...
                    // line number didn't change, so just move one to the next o
ne
                    p+= *(uint16_t *)p;
                }
                // end of program?
                ...
                tokenBuffer = p+4;
                // if the target for a jump is missing (e.g. line deleted) and w
e're on the next line
                // reset the stmt number to 0
                if (jumpLineNumber && jumpStmtNumber && lineNumber > jumpLineNum
ber)
                    jumpStmtNumber = 0;
            }
            if (jumpStmtNumber)
